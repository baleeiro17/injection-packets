...
from utils.regex import checa_igmp, checa_igmp_member, checa_multicast_vlan, checa_vlan_voip, get_svlan_serviceport
            ...
            for porta in portas:
                if porta.split("/")[0] == slot:
                    ...
                    for index in range(128, 576, 64):
                        file.write("gemport add " + porta.split("/")[1] + " gemportid " + str(index) + "-" + str(index + 63) + " eth encrypt on\n")
                    file.write("\n")
    ...
    for slot in slots:
        ...
        for porta in portas:
              if porta.split("/")[0] == slot:
                    # configura os clientes por porta
                    for index in range(128, 576, 64):
                        aplicacao.append("gemport add " + porta.split("/")[1] + " gemportid " + str(index) + "-" + str(index + 63) + " eth encrypt on\n")
                    # file.write("\n")
                    for cliente in clientes:
                        try:
                            if cliente['portapara'].split("/")[1].strip() == porta.split("/")[0] \
                                ...
                                for serviceport in cliente['serviceportpara']:
                                    if checa_vlan_voip(serviceport['configserviceport'], vlanvoipantiga):
                                        config = serviceport['configserviceport'].split(" " + vlanvoipantiga + " ")[0] \
                                            + " " + vlanvoipnova + " " + serviceport['configserviceport'].split(" " + vlanvoipantiga + " ")[1]
                                    else:
                                        config = serviceport['configserviceport']
                                    aplicacao.append(config + "\n")
                                # file.write("\n")
...
def gera_proc_corrigido_huawei(clientes, hostname, vlanrede, vlanvoip, depara):
    ...
    slots = mostra_slots_depara_antes(depara)
    portas = mostra_portas_slots_depara_antes(depara)
        ...
        for slot in slots:
            for porta in portas:
                if porta.split("/")[0] == slot:
                    ...
                    for cliente in clientes:
                        try:
                            if cliente['portade'].split("/")[1].strip() == slot \
                                    and cliente['portade'].split("/")[2].strip() == porta.split("/")[1]:
                                if relatorios_falta_config(cliente):
                                    if len(cliente['serviceportpara']) > 0:
                                        file.write(
                                            "interface gpon 0/" + get_slot_destino(slot, depara) + "\n")
                                        # file.write("\n")
                                        configs = cliente['configinterface'].splitlines(
                                        )
                                        for config in configs[1:]:
                                            file.write(config + "\n")
                                    for serviceportpara in cliente['serviceportpara']:
                                        svlan_valid = get_svlan_serviceport(
                                            serviceportpara['configserviceport'])
                                        if svlan_valid['valid']:
                                            if svlan_valid['value'] == vlanrede:
                                            ...
                                            elif svlan_valid['value'] == vlanvoip:
                                                file.write("ont port vlan " + cliente['portapara'].split("/")[2].strip(
